...
exports.login = async (req, res) => {
  try {
    ...
    // If permissions are not set or are default (all false), set them based on role
    const hasPermissions = user.permissions && 
      (user.permissions.employees.view !== undefined || 
       user.permissions.assets.view !== undefined);
    
    if (!hasPermissions) {
      user.permissions = setPermissionsByRole(user.role);
    }
...
exports.getMe = async (req, res) => {
  try {
    ...
    // Ensure mfaEnabled field exists and has a default value
